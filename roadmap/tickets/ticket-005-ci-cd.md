...
## Resolution
- Created [deploy.yml](file:///Users/ilembitov/Projects/bios/.github/workflows/deploy.yml) under `.github/workflows/`.
- Configured checkout and setup-pages actions to run on pushes to `main` branch.
- Added step to download and install exact Hugo extended binary (`v0.163.3`).
- Added compilation step setting dynamic base URL:
  `hugo --minify --source site --baseURL "${{ steps.pages.outputs.base_url }}/"`
- Wired build artifacts to upload and deploy to GitHub Pages.
